...
class CMSService:
    ...
    async def update_content(self, content_id: str, content_data: Dict[str, Any], plugin_name: Optional[str] = None) -> Optional[Dict[str, Any]]:
    ...
    async def list_content(self, plugin_name: Optional[str] = None, limit: int = 100, offset: int = 0) -> List[Dict[str, Any]]:
        """List all content"""
        plugin = self.plugin_registry.get_plugin(plugin_name)
        if not plugin:
            raise ValueError(f"Plugin '{plugin_name}' not found")
