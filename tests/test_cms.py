...
def test_get_content(client, auth_headers):
    ...
    create_response = client.post(
        "/api/v1/contents", json={"title": "Test Content", "body": "This is test content"}, headers=auth_headers
    )
    content_id = create_response.json()["id"]

    # Then get it
    response = client.get(f"/api/v1/contents/{content_id}", headers=auth_headers)
    assert response.status_code == status.HTTP_200_OK
    data = response.json()
...
def test_update_content(client, auth_headers):
    ...
    # Create content
    create_response = client.post(
        "/api/v1/contents", json={"title": "Original Title", "body": "Original body"}, headers=auth_headers
    )
    content_id = create_response.json()["id"]
...
def test_delete_content(client, auth_headers):
    ...
    # Create content
    create_response = client.post(
        "/api/v1/contents", json={"title": "To Delete", "body": "This will be deleted"}, headers=auth_headers
    )
    content_id = create_response.json()["id"]

    # Delete it
    response = client.delete(f"/api/v1/contents/{content_id}", headers=auth_headers)
    assert response.status_code == status.HTTP_204_NO_CONTENT

    # Verify it's deleted
    get_response = client.get(f"/api/v1/contents/{content_id}", headers=auth_headers)
...
def test_list_content(client, auth_headers):
    ...
    # Create some content
    for i in range(3):
        client.post("/api/v1/contents", json={"title": f"Content {i}", "body": f"Body {i}"}, headers=auth_headers)

    # List it
    response = client.get("/api/v1/contents", headers=auth_headers)
    assert response.status_code == status.HTTP_200_OK
    data = response.json()
    assert len(data) >= 3
